...
import { loadObservedState, requestKill, verifyObserved } from '../core/runtime.js';
    ...
    for (;;) {
      ...
      try {
        if (msg.method === 'initialize') rpc(msg.id, { protocolVersion: '2024-11-05', capabilities: { tools: {} }, serverInfo: { name: 'swarmwatch', version: '0.1.0' } });
          ...
          { name: 'swarm_ingest', description: 'Append one swarm event.', inputSchema: { type: 'object', properties: { type: { type: 'string' }, agentId: { type: 'string' }, targetAgentId: { type: 'string' }, costUsd: { type: 'number' }, tokens: { type: 'number' }, message: { type: 'string' } }, required: ['type', 'agentId'] } },
